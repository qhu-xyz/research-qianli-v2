"""Comprehensive tests for run_v9_bands.py.

Tests asymmetric calibration, band application, experiment runner, winner
selection, edge cases, and pipeline gate compatibility.

v9 removes correction (bidirectional + cold-start), so no correction tests.

Usage:
    cd /home/xyz/workspace/pmodel && source .venv/bin/activate
    python /home/xyz/workspace/research-qianli-v2/research-annual/scripts/test_v9_bands.py
"""
...
def test_pipeline_gate_structure():
    ...
    # Build metrics dict as run_round_v9 would
    metrics = {
        "coverage": {
...
# ─── Test 22: No correction parameters in v9 ─────────────────────────────────

def test_no_correction_parameters():
    """v9 removed correction. Verify the function signature has no apply_correction."""
    print("\n[Test 22] No correction parameters in v9")
    import inspect
    ...
    check("'apply_correction' not in run_experiment params",
          "apply_correction" not in params)
    check("'cv_mode' in run_experiment params",
          "cv_mode" in params)
    check("'min_train_pys' in run_experiment params",
          "min_train_pys" in params)

    # Default min_train_pys should be 3
    default = sig.parameters["min_train_pys"].default
    check("min_train_pys default is 3", default == 3,
          f"actual default={default}")


# ─── Test 23: min_train_pys filtering ─────────────────────────────────────────

def test_min_train_pys_filtering():
    """Verify min_train_pys=3 excludes early folds from aggregation."""
    print("\n[Test 23] min_train_pys filtering")
    df = make_df(n=30000, n_pys=6)
    pys = sorted(df["planning_year"].unique().to_list())

    # With min_train_pys=3, first 2 temporal folds (1 PY, 2 PYs) should be excluded
    result = run_experiment(
        df, "aq1", pys, 4, "baseline", "test_filter",
        cv_mode="temporal", min_train_pys=3,
    )

    stab = result["stability"]
    check("n_folds_total > n_folds_filtered",
          stab["n_folds_total"] > stab["n_folds_filtered"],
          f"total={stab['n_folds_total']}, filtered={stab['n_folds_filtered']}")

    # Check that per_py entries have meets_min_train_pys correctly set
    n_excluded = 0
    for py_str, py_data in result["per_py"].items():
        train_pys = py_data.get("train_pys", [])
        meets = py_data.get("meets_min_train_pys", True)
        if len(train_pys) < 3:
            check(f"PY {py_str} ({len(train_pys)} train PYs) excluded",
                  not meets)
            n_excluded += 1
        else:
            check(f"PY {py_str} ({len(train_pys)} train PYs) included",
                  meets)

    check("at least 1 fold excluded by min_train_pys=3", n_excluded > 0,
...
def main():
    print("=" * 70)
    print("  test_v9_bands.py — Comprehensive tests for v9 bands")
    print("=" * 70)
    ...
    test_no_correction_parameters()      # Test 22
    test_min_train_pys_filtering()       # Test 23
