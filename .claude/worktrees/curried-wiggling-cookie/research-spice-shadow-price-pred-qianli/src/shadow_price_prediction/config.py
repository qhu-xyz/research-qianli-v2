...
class FeatureConfig:
    ...
    step1_features: list[tuple[str, int]] = field(
        default_factory=lambda: [
            ...
            ("prob_below_95", -1),
            ("prob_below_90", -1),
            # --- Severity signal (v007, orthogonal to exceedance probs) ---
            ("expected_overload", 1),
    ...
    step2_features: list[tuple[str, int]] = field(
        default_factory=lambda: [
            # --- Density exceedance probabilities ---
            ("prob_exceed_110", 1),
            ("prob_exceed_105", 1),
            ("prob_exceed_100", 1),
            ("prob_exceed_95", 1),
            ...
            ("prob_below_95", -1),
            ("prob_below_90", -1),
            # --- Severity & shape features (v007) ---
            ("expected_overload", 1),
            ("tail_concentration", 1),
            ("prob_band_95_100", 1),
            ("prob_band_100_105", 1),
            # --- Density distribution moments ---
            ("density_mean", 0),
            ("density_variance", 0),
            ("density_skewness", 0),
            ("density_kurtosis", 0),
...
class EnsembleConfig:
    ...
    # Default classifier ensemble — v008: simpler model (v000 core + regularization)
    ...
    # Branch-specific classifier ensemble — v008: simpler model
...
class TrainingConfig:
    ...
    train_months: int = 10  # Months of data used for model fitting
    val_months: int = 2  # Months of data used for threshold optimization / validation
    test_months: int = 0  # Months of data reserved as holdout (not used during training)
    ...
    def train_months_lookback(self) -> int:
        """Total months of data to load (train + val + test)."""
        return self.train_months + self.val_months + self.test_months
