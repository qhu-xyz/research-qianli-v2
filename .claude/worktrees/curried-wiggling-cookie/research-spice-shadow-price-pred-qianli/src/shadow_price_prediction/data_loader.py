...
class BaseDataLoader(ABC):
    ...
    def get_spice_map(self, cons_path: Path) -> pd.DataFrame:
        ...
        Also extracts constraint metadata: type (interface/line) and limit (MW).
        """
        cons = fetch_constraints(cons_path)
        valid = cons[cons["convention"] != 999].dropna(subset=["branch_name"])
        gp = valid.groupby("constraint_id")
        spice_map = pd.concat([
            gp["branch_name"].apply(",".join),
            gp["type"].first(),
            gp["limit"].first(),
        ], axis=1)
        return spice_map

    def compute_sf_features(self, sf_path: Path, constraint_ids: pd.Index) -> pd.DataFrame:
        """
        Compute shift-factor statistics per constraint from sf.parquet.

        For each constraint column in the SF matrix, computes:
        - sf_max_abs: maximum absolute shift factor (peak node sensitivity)
        - sf_mean_abs: mean absolute shift factor (average sensitivity)
        - sf_std: standard deviation of shift factors (sensitivity spread)
        - sf_nonzero_frac: fraction of nodes with |SF| > 0.001 (constraint reach)

        Parameters
        ----------
        sf_path : Path
            Directory containing sf.parquet
        constraint_ids : pd.Index
            Constraint IDs to compute features for (from density data)

        Returns
        -------
        pd.DataFrame
            Index: constraint_id, columns: sf_max_abs, sf_mean_abs, sf_std, sf_nonzero_frac
        """
        sf_file = sf_path / "sf.parquet"
        if not sf_file.exists():
            # Return empty features if SF data not available
            return pd.DataFrame(
                0.0,
                index=constraint_ids,
                columns=["sf_max_abs", "sf_mean_abs", "sf_std", "sf_nonzero_frac"],
            )

        # Read only the columns we need (constraint_ids present in density)
        # Filter to columns that actually exist in the parquet to avoid KeyError
        str_ids = [str(c) for c in constraint_ids]
        available_cols = set(pq.read_schema(sf_file).names)
        cols_to_read = [c for c in str_ids if c in available_cols]
        if not cols_to_read:
            return pd.DataFrame(
                0.0,
                index=constraint_ids,
        ...
        sf = pd.read_parquet(sf_file, columns=cols_to_read)

        # Compute per-constraint (per-column) statistics
        abs_sf = sf.abs()
        result = pd.DataFrame({
            "sf_max_abs": abs_sf.max(),
            "sf_mean_abs": abs_sf.mean(),
            "sf_std": sf.std(),
            "sf_nonzero_frac": (abs_sf > 0.001).mean(),
        })
        result.index.name = "constraint_id"
    ...
    def load_data_for_outage(
        ...
        def calculate_direction_features(df, direction):
            ...
            if len(existing_indices) > 1:
                ...
                # --- v007: expected_overload (severity signal) ---
                if direction == 1:
                    ol_mask = x_vals > 1.0
                else:
                    ol_mask = x_vals < -1.0
                ol_x = x_vals[ol_mask]
                if len(ol_x) > 1:
                    ol_cols = [moments_cols[i] for i in range(len(moments_cols)) if ol_mask[i]]
                    ol_y = df[ol_cols].values
                    res["expected_overload"] = np.trapezoid(y=np.abs(ol_x) * ol_y, x=ol_x, axis=1)
                else:
                    res["expected_overload"] = 0.0

                # --- v007: density_entropy (distribution uncertainty) ---
                y_clipped = np.clip(y_vals, 1e-12, None)
                res["density_entropy"] = -np.trapezoid(y=y_clipped * np.log(y_clipped), x=x_vals, axis=1)

                # --- v007: density_cv (scale-free volatility) ---
                res["density_cv"] = np.sqrt(variance) / (np.abs(density_mean) + 1e-9)
            else:
                ...
                res["expected_overload"] = 0.0
                res["density_entropy"] = 0.0
                res["density_cv"] = 0.0

            # --- v007: derived probability features ---
            res["tail_concentration"] = res["prob_exceed_100"] / (res["prob_exceed_80"] + 1e-9)
            res["prob_band_95_100"] = (res["prob_exceed_95"] - res["prob_exceed_100"]).clip(lower=0)
            res["prob_band_100_105"] = (res["prob_exceed_100"] - res["prob_exceed_105"]).clip(lower=0)
        ...
        sf_template = self.config.iso.data_paths.sf_path_template
        if sf_template:
            sf_path = Path(
                sf_template.format(
                    auction_month=auction_month.strftime("%Y-%m"),
            ...
            sf_features = self.compute_sf_features(sf_path, density_df.index)
            # Join on constraint_id (full_df index inherits from density_df)
            for col in sf_features.columns:
                full_df[col] = full_df.index.get_level_values("constraint_id").map(sf_features[col]).fillna(0)
            del sf_features

        # --- Constraint metadata features ---
        if spice_map is not None:
            cid = full_df.index.get_level_values("constraint_id")
            mapped_type = pd.Series(cid.map(spice_map["type"]), index=full_df.index)
            full_df["is_interface"] = (mapped_type == "flowgate_nomogram").astype(float)
            raw_limit = pd.Series(cid.map(spice_map["limit"]), index=full_df.index).fillna(0).astype(float)
            full_df["constraint_limit"] = np.log1p(raw_limit.clip(lower=0))
        ...
        # --- v007: historical DA trend and max season ---
        full_df["hist_da_trend"] = full_df["recent_hist_da"] / (full_df[cols].mean(axis=1) + 1e-3)
        full_df["hist_da_max_season"] = full_df[cols].max(axis=1)
