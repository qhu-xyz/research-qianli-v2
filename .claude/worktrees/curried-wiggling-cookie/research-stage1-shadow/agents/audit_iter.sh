...
audit_iter() {
  ...
  if [[ -f "$COMP_JSON" && -n "$VERSION_ID" ]]; then
    ...
    if [[ "$BRIEF" == "true" ]]; then
      ...
      local AUC AP NDCG BRIER
      AUC=$(jq -r ".versions[\"${VERSION_ID}\"][\"S1-AUC\"].mean_value // \"?\"" "$COMP_JSON" 2>/dev/null)
      AP=$(jq -r ".versions[\"${VERSION_ID}\"][\"S1-AP\"].mean_value // \"?\"" "$COMP_JSON" 2>/dev/null)
      NDCG=$(jq -r ".versions[\"${VERSION_ID}\"][\"S1-NDCG\"].mean_value // \"?\"" "$COMP_JSON" 2>/dev/null)
      BRIER=$(jq -r ".versions[\"${VERSION_ID}\"][\"S1-BRIER\"].mean_value // \"?\"" "$COMP_JSON" 2>/dev/null)
      echo "  Gates: A=${GA} B=${GB} | AUC=${AUC} AP=${AP} NDCG=${NDCG} BRIER=${BRIER}"
    else
      echo ""
      ...
      echo "   |------|------|-------|----|--------|----|------|----|---------|"
      for GATE in S1-AUC S1-AP S1-VCAP@100 S1-NDCG S1-BRIER S1-VCAP@500 S1-VCAP@1000 S1-REC S1-CAP@100 S1-CAP@500; do
...
audit_batch() {
  ...
  echo ""
  printf "  %-8s | %-8s | %-8s | %-8s | %-8s | %-8s\n" "Iter" "Version" "AUC" "AP" "NDCG" "BRIER"
  printf "  %-8s-+-%-8s-+-%-8s-+-%-8s-+-%-8s-+-%-8s\n" "--------" "--------" "--------" "--------" "--------" "--------"

  # v0 baseline row
  local V0_JSON="${PROJECT_DIR}/registry/comparisons/${BATCH_ID}_iter1.json"
  if [[ -f "$V0_JSON" ]]; then
    local V0_AUC V0_AP V0_NDCG V0_BRIER
    V0_AUC=$(jq -r '.versions["v0"]["S1-AUC"].mean_value // "?"' "$V0_JSON" 2>/dev/null)
    V0_AP=$(jq -r '.versions["v0"]["S1-AP"].mean_value // "?"' "$V0_JSON" 2>/dev/null)
    V0_NDCG=$(jq -r '.versions["v0"]["S1-NDCG"].mean_value // "?"' "$V0_JSON" 2>/dev/null)
    V0_BRIER=$(jq -r '.versions["v0"]["S1-BRIER"].mean_value // "?"' "$V0_JSON" 2>/dev/null)
    printf "  %-8s | %-8s | %-8s | %-8s | %-8s | %-8s\n" "base" "v0" "$V0_AUC" "$V0_AP" "$V0_NDCG" "$V0_BRIER"
  fi
  ...
  for I in $(seq 1 "$MAX_N"); do
    ...
    if [[ -f "$CJ" ]]; then
      ...
      if [[ "$VID" != "?" && "$VID" != "null" ]]; then
        local I_AUC I_AP I_NDCG I_BRIER
        I_AUC=$(jq -r ".versions[\"${VID}\"][\"S1-AUC\"].mean_value // \"?\"" "$CJ" 2>/dev/null)
        I_AP=$(jq -r ".versions[\"${VID}\"][\"S1-AP\"].mean_value // \"?\"" "$CJ" 2>/dev/null)
        I_NDCG=$(jq -r ".versions[\"${VID}\"][\"S1-NDCG\"].mean_value // \"?\"" "$CJ" 2>/dev/null)
        I_BRIER=$(jq -r ".versions[\"${VID}\"][\"S1-BRIER\"].mean_value // \"?\"" "$CJ" 2>/dev/null)
        printf "  %-8s | %-8s | %-8s | %-8s | %-8s | %-8s\n" "iter${I}" "$VID" "$I_AUC" "$I_AP" "$I_NDCG" "$I_BRIER"
      fi
    fi
  done
