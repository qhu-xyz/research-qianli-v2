"""Bands v5: Asymmetric Band Calibration.

v4 passes all HARD gates but inflates widths +1-2% vs promoted v3. The width
inflation comes from symmetric bands over-covering the thin tail on skewed
residual distributions.

v5 uses signed quantile pairs instead of absolute quantiles:
    symmetric:   band = [baseline - w, baseline + w]   where w = quantile(|r|, level)
    asymmetric:  band = [baseline + lo, baseline + hi] where lo = quantile(r, (1-level)/2)
                                                             hi = quantile(r, (1+level)/2)

For skewed distributions, the asymmetric interval is strictly narrower because it
avoids wasting width on the thin tail. Half-width = (hi - lo) / 2 is directly
comparable to the symmetric w for BG2 gate checks.

Also tests higher bin counts (6, 8) to further reduce within-bin heterogeneity.

Experiments per round:
    R1 (LOO):      sym_4b_cs, asym_4b_cs, asym_6b_cs, asym_8b_cs
    R2 (temporal):  sym_6b_cs, asym_6b_cs, asym_8b_cs
    R3 (temporal):  sym_6b_cs, asym_6b_cs, asym_8b_cs

Winner = lowest mean P95 half-width across quarters, subject to BG1 pass.

Usage:
    cd /home/xyz/workspace/pmodel && source .venv/bin/activate
    python /home/xyz/workspace/research-qianli-v2/research-annual/scripts/run_v5_bands.py
"""
...
# ─── Shared functions (inlined from v1-v4 scripts) ──────────────────────────
...
def run_round_v5(
    ...
    """Run v5 experiments for one round.

    Each experiment dict: {"name": str, "n_bins": int, "symmetric": bool}
    ...
    print(f"\n{'#'*80}")
    print(f"  ROUND {round_num} — v5 ASYMMETRIC BAND EXPERIMENTS")
    ...
    config_data = {
        "schema_version": 1,
        "version": version_id,
        "description": f"R{round_num} {band_type} bands ({winner_exp['n_bins']} quantile bins, class+sign stratified)",
    ...
    print(f"\n{'='*70}")
    print(f"  R{round_num} v5 SUMMARY")
    print(f"{'='*70}")
    print(f"\n  Winner: {winner_name} ({band_type}, {winner_exp['n_bins']} bins)")

    gate_label = "temporal, min_train_pys=1"
    ...
    if prior_version_part:
        ...
        if prior_metrics_path.exists():
            ...
            print(f"  {'Quarter':<10} {'Prior hw':>10} {'v5 hw':>10} {'Change':>10}")
            print(f"  {'-'*10} {'-'*10} {'-'*10} {'-'*10}")
            for q in QUARTERS:
                prior_w = prior_metrics["widths"][q]["overall"]["p95"]["mean_width"]
                v5_w = metrics["widths"][q]["overall"]["p95"]["mean_width"]
                pct = round((v5_w - prior_w) / prior_w * 100, 1) if prior_w else 0
                print(f"  {q:<10} {prior_w:>10.0f} {v5_w:>10.1f} {pct:>+9.1f}%")
...
def main():
    print(f"Bands v5: Asymmetric Band Calibration")
    print(f"Memory at start: {mem_mb():.0f} MB")

    # ─── R1: LOO primary, 4 experiments ──────────────────────────────

    r1_experiments = [
        {"name": "sym_4b_cs",  "n_bins": 4, "symmetric": True},
        {"name": "asym_4b_cs", "n_bins": 4, "symmetric": False},
        {"name": "asym_6b_cs", "n_bins": 6, "symmetric": False},
        {"name": "asym_8b_cs", "n_bins": 8, "symmetric": False},
    ]
    ...
    r1_metrics = run_round_v5(
        round_num=1, experiments=r1_experiments, baseline_col="nodal_f0",
        data_loader=r1_loader, pys=R1_PYS,
        version_id="v5",
        prior_version_part="bands/v4/r1",
    )
    gc.collect()
    print(f"\nMemory after R1: {mem_mb():.0f} MB")

    # ─── R2: Temporal primary, 3 experiments ─────────────────────────

    r2_experiments = [
        {"name": "sym_6b_cs",  "n_bins": 6, "symmetric": True},
        {"name": "asym_6b_cs", "n_bins": 6, "symmetric": False},
        {"name": "asym_8b_cs", "n_bins": 8, "symmetric": False},
    ]
    ...
    r2_metrics = run_round_v5(
        round_num=2, experiments=r2_experiments, baseline_col="mtm_1st_mean",
        data_loader=r2_loader, pys=R2R3_PYS,
        version_id="v5",
        prior_version_part="bands/v4/r2",
    )
    gc.collect()
    print(f"\nMemory after R2: {mem_mb():.0f} MB")

    # ─── R3: Temporal primary, 3 experiments ─────────────────────────
    ...
    r3_metrics = run_round_v5(
        round_num=3, experiments=r3_experiments, baseline_col="mtm_1st_mean",
        data_loader=r3_loader, pys=R2R3_PYS,
        version_id="v5",
        prior_version_part="bands/v4/r3",
    )
    gc.collect()
