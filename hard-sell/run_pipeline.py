"""
MISO Sell April 2026 (auc2604) - Pipeline Runner

Normal sells only (no constraints, no scenario pricing).
Reuses March V5 params for f0/f1.

Output dir: /opt/temp/qianli/miso/apr_v1/
"""
...
import numpy as np
import pandas as pd
from miso_sell import generate_normal_sell_files
...
OUTPUT_DIR = "/opt/temp/qianli/miso/apr_v1"
AUCTION = "auc2604"
VERSION = "v1"
AUCTION_ROUND = 1
...
def run_pipeline(
    ...
    for period_type in period_types:
        for class_type in class_types:
            ...
            try:
                result = generate_normal_sell_files(
                    data_path=data_path,
...
def print_summary(results):
    print(f"\n{'=' * 60}")
    print("SUMMARY - MISO Sell Apr 2026 (auc2604)")
    print(f"{'=' * 60}")
    ...
    for (period_type, class_type), result in sorted(results.items()):
        if "error" in result:
            ...
            print(f"{period_type} {class_type}: {orig:4d} trades")
            total_original += orig

    print(f"{'-' * 60}")
    print(f"TOTAL:       {total_original:4d} trades")
    ...
    print("Expected: 40 (5 strategies x 2 flows x 4 slices)")
