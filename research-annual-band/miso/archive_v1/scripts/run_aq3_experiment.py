...
#!/usr/bin/env python3
"""
aq3 (Dec-Feb) R1 baseline experiment — comprehensive comparison.

Tests all available baseline sources for predicting R1 aq3 MCP:
  1. H              — historical DA congestion (current production)
  2. Prior year R3   — PY-1 R3 aq3 path MCP
  3. Prior year R2   — PY-1 R2 aq3 path MCP
  4. Prior year R1   — PY-1 R1 aq3 path MCP
  5. Nodal f0        — nodal stitched from 3-month avg, corrected year
  6. f0 path corr    — prior year f0 path avg for Dec/Jan/Feb (corrected)
  7. f1 path prior   — prior year f1 avg for Dec/Jan/Feb
  8. q3 path         — prior year q3 quarterly forward MCP (NEW for aq3)

CRITICAL: aq3 crosses the calendar year boundary.
  PY 2024 aq3 delivers Dec 2024, Jan 2025, Feb 2025.
  Year mapping is MIXED:
    Dec (month 12, >= 6): dy = PY - 1  → Dec 2023 for PY 2024
    Jan (month 1,  <  6): dy = PY      → Jan 2024 for PY 2024
    Feb (month 2,  <  6): dy = PY      → Feb 2024 for PY 2024
  Replacement target date: {PY+1}-02 (last delivery month in PY+1 calendar year).
  q3 is NOT in f0p_cleared_all.parquet — loaded from cleared trades.

Usage:
    cd /home/xyz/workspace/pmodel && source .venv/bin/activate
    python /home/xyz/workspace/research-qianli-v2/research-annual/scripts/run_aq3_experiment.py
"""
...
def year_map_aq3(py: int, m: int) -> int:
    """Year mapping for aq3 delivery months. Mixed: Dec=PY-1, Jan/Feb=PY."""
    if m >= 6:  # Dec
        return py - 1
    else:       # Jan, Feb
        return py
...
# ======================================================================
# STEP 1: Load aq3 R1 paths from existing Phase 2 output
# ======================================================================
print(f"{'='*80}")
print(f"STEP 1: Load aq3 R1 paths | mem: {mem_mb():.0f} MB")
print(f"{'='*80}")

r1_all = pl.read_parquet(f"{WORK_DIR}/r1_with_nodal.parquet")
aq3 = r1_all.filter(pl.col("period_type") == "aq3").clone()
del r1_all
gc.collect()

# Drop nodal_r3 (buggy column from Phase 2, bug proven in aq1)
if "nodal_r3" in aq3.columns:
    aq3 = aq3.drop("nodal_r3")

total_n = aq3.height
pys = sorted(aq3["planning_year"].unique().to_list())
n_pys = len(pys)
n_pys_with_prior = len([py for py in pys if int(py) > 2019])
print(f"  aq3 rows: {total_n:,}")
...
for col in ["mtm_1st_mean", "prior_r3_path"]:
    if col in aq3.columns:
        cov = aq3.filter(pl.col(col).is_not_null()).height
...
paths_aq3 = (
    all_res.filter(pl.col("period_type") == "aq3")
...
r2_prior = (
    paths_aq3.filter(pl.col("round") == 2)
    .select(
...
r1_prior = (
    paths_aq3.filter(pl.col("round") == 1)
    .select(
        (pl.col("planning_year") + 1).alias("planning_year"),
        "class_type", "source_id", "sink_id",
...
aq3 = aq3.cast({"planning_year": pl.Int64})
...
pre = aq3.height
aq3 = aq3.join(r2_prior, on=join_keys, how="left")
assert aq3.height == pre, f"Row count changed after R2 join: {pre} -> {aq3.height}"
aq3 = aq3.join(r1_prior, on=join_keys, how="left")
assert aq3.height == pre, f"Row count changed after R1 join: {pre} -> {aq3.height}"
...
# ======================================================================
# STEP 3: f0 path-level with corrected MIXED year mapping
# ======================================================================
print(f"\n{'='*80}")
print(f"STEP 3: f0 path-level cross-product (MIXED year mapping) | mem: {mem_mb():.0f} MB")
print(f"{'='*80}")

# aq3 delivers Dec/Jan/Feb.  Dec data is in month 12, Jan in month 1, Feb in month 2.
f0 = (
    pl.scan_parquet(f"{DATA_DIR}/f0p_cleared_all.parquet")
    .filter(
        (pl.col("period_type") == "f0")
        & pl.col("split_market_month").dt.month().is_in(AQ3_MONTHS)
    )
    .select(
...
print(f"  f0 Dec/Jan/Feb rows: {f0.height:,}")
...
# Year mapping: MIXED for aq3
#   Dec (month 12, >= 6): dy = PY - 1
#   Jan (month 1,  <  6): dy = PY
#   Feb (month 2,  <  6): dy = PY
print(f"\n  Year mapping verification (MIXED):")
print(f"  {'PY':>6} | {'Month':>5} | {'dy':>6} | {'Actual date':>15}")
for py in pys:
    for m in AQ3_MONTHS:
        dy = year_map_aq3(int(py), m)
        avail = "Y" if dy in dy_range else "N"
        # Actual delivery: Dec in PY calendar year, Jan/Feb in PY+1
        actual_yr = int(py) if m == 12 else int(py) + 1
        print(f"  {int(py):>6} | {m:>5} | {dy:>4} [{avail}] | {actual_yr}-{m:02d}")

map_corrected = []
for py in pys:
    for m in AQ3_MONTHS:
        dy = year_map_aq3(int(py), m)
        map_corrected.append({"planning_year": int(py), "dy": dy, "dm_num": int(m)})
...
pre = aq3.height
aq3 = aq3.join(f0_avg_c.drop("_nc"), on=join_keys, how="left")
assert aq3.height == pre, f"Row count changed after f0 join: {pre} -> {aq3.height}"
del f0_avg_c
gc.collect()

print(f"\n  f0 path coverage by PY:")
for py in pys:
    sub = aq3.filter(pl.col("planning_year") == py)
...
# ======================================================================
# STEP 4: f1 prior year path-level (same mixed year mapping)
# ======================================================================
print(f"\n{'='*80}")
print(f"STEP 4: f1 prior year (mixed year mapping) | mem: {mem_mb():.0f} MB")
print(f"{'='*80}")
...
print(f"  f1 Dec/Jan/Feb rows: {f1.height:,}")
...
if f1.height > 0:
    ...
    # Same mixed year mapping as f0
    map_f1 = []
    for py in pys:
        for m in AQ3_MONTHS:
            dy = year_map_aq3(int(py), m)
            map_f1.append({"planning_year": int(py), "dy": dy, "dm_num": int(m)})
    ...
    pre = aq3.height
    aq3 = aq3.join(f1_avg.cast({"planning_year": pl.Int64}), on=join_keys, how="left")
    assert aq3.height == pre, f"Row count changed after f1 join: {pre} -> {aq3.height}"
    del f1_avg
    gc.collect()

    cov = aq3.filter(pl.col("f1_path").is_not_null()).height
    print(f"  f1 path coverage: {cov:,} / {total_n:,} ({cov/total_n*100:.1f}%)")
else:
    aq3 = aq3.with_columns(pl.lit(None).cast(pl.Float64).alias("f1_path"))
    print(f"  No f1 data for aq3 months")
    del f1
    gc.collect()
print(f"  mem: {mem_mb():.0f} MB")
...
# ======================================================================
# STEP 5: Ray session — q3 quarterly forward + Nodal f0 stitching
# ======================================================================
print(f"\n{'='*80}")
print(f"STEP 5: Ray session (q3 + Nodal f0) | mem: {mem_mb():.0f} MB")
print(f"{'='*80}")
...
# --- 5a: q3 quarterly forward MCPs ---
print(f"\n  --- Step 5a: q3 quarterly forward ---")
# q3 is auctioned in July and October. Delivers Dec/Jan/Feb (crosses calendar year).
# q3 is NOT in f0p_cleared_all.parquet — must load from cleared trades.
# For PY N aq3, we want prior year q3: delivers Dec(PY-1)/Jan(PY)/Feb(PY).
# That q3 was auctioned July or October of PY-1.

miso_aptools = MisoApTools()
q3_slices: list[pl.DataFrame] = []

for py in pys:
    prior_year = int(py) - 1
    if prior_year < 2019:
        print(f"  PY {int(py)}: skip q3 (prior year {prior_year} < 2019)")
        continue

    # q3 delivers Dec of prior_year through Feb of prior_year+1.
    # Load trades covering that delivery window.
    # The auction happens in July/October of prior_year.
    trades = miso_aptools.get_all_cleared_trades(
        start_date=pd.Timestamp(f"{prior_year}-07-01"),
        end_date=pd.Timestamp(f"{prior_year + 1}-06-01"),
    )
    q3_trades = trades[trades["period_type"] == "q3"].copy()
    miso_aptools.tools.cast_category_to_str(q3_trades)
    del trades
    gc.collect()

    n_q3 = len(q3_trades)
    print(f"  PY {int(py)}: {n_q3:,} q3 trades from {prior_year}")
    ...
    if n_q3 > 0:
        ...
        agg_pdf = (
            ...
            .mean()
            .reset_index()
            .rename(columns={"mcp": "q3_path"})
        )
        agg_pdf["planning_year"] = int(py)
        py_slice = pl.from_pandas(
            agg_pdf[["planning_year", "class_type", "source_id", "sink_id", "q3_path"]]
        )
        q3_slices.append(py_slice)
        del agg_pdf
...
if q3_slices:
    q3_df = pl.concat(q3_slices).cast({
    ...
    pre = aq3.height
    aq3 = aq3.join(q3_df, on=join_keys, how="left")
    assert aq3.height == pre, f"Row count changed after q3 join: {pre} -> {aq3.height}"
    del q3_df
    gc.collect()

    cov = aq3.filter(pl.col("q3_path").is_not_null()).height
    print(f"  q3 path coverage: {cov:,} / {total_n:,} ({cov/total_n*100:.1f}%)")
else:
    del q3_slices
    gc.collect()
    aq3 = aq3.with_columns(pl.lit(None).cast(pl.Float64).alias("q3_path"))
    print(f"  No q3 data found")
print(f"  mem: {mem_mb():.0f} MB")


# --- 5b: Nodal f0 stitching (MIXED year, 3-month avg) ---
print(f"\n  --- Step 5b: Nodal f0 stitching (mixed year) ---")
...
for py in pys:
    ...
    # For aq3, target date = {PY+1}-02 (last delivery month is Feb of PY+1)
    target_date = f"{int(py) + 1}-02"
    ...
    # MIXED market months: Dec from PY-1, Jan/Feb from PY
    for m in AQ3_MONTHS:
        dy = year_map_aq3(int(py), m)
        mm = f"{dy}-{m:02d}"
        try:
...
pre = aq3.height
aq3 = aq3.join(src_lookup, on=nj_keys + ["source_id"], how="left")
assert aq3.height == pre, f"Row count changed after src join: {pre} -> {aq3.height}"
aq3 = aq3.join(snk_lookup, on=nj_keys + ["sink_id"], how="left")
assert aq3.height == pre, f"Row count changed after snk join: {pre} -> {aq3.height}"
del src_lookup, snk_lookup
gc.collect()

aq3 = aq3.with_columns(
...
cov = aq3.filter(pl.col("nodal_f0").is_not_null()).height
...
# Deduplicate to unique paths per month (take first MCP for each unique path)
f0_monthly_dedup = (
    f0_monthly
    .group_by(["dy", "dm_num", "class_type", "source_id", "sink_id"])
...
# Build per-month nodal source/sink lookups from nodal_dedup
...
# Map (PY, month) -> (dy, dm_num) for joining with f0 path data
pm_map = []
for py in pys:
    for m in AQ3_MONTHS:
        dy = year_map_aq3(int(py), m)
        pm_map.append({"planning_year": int(py), "month": int(m), "dy": dy, "dm_num": int(m)})
pm_df = pl.DataFrame(pm_map).cast({
    "planning_year": pl.Int64, "month": pl.Int8,
    "dy": pl.Int16, "dm_num": pl.Int8,
})
...
for m in AQ3_MONTHS:
    for py in pys:
        ...
        # Get per-month nodal source/sink
        ...
        # Join: f0 paths + nodal source + nodal sink
        check = (
            f0_m
...
del f0_monthly_dedup, src_monthly, snk_monthly, pm_df
gc.collect()

# --- 3-month averaged verification ---
print(f"\n  --- 3-month averaged verification ---")
both = aq3.filter(pl.col("f0_path_corr").is_not_null() & pl.col("nodal_f0").is_not_null())
n_both = both.height
...
# ======================================================================
print(f"\n{'='*80}")
print(f"RESULTS: aq3 (Dec-Feb) R1 Baseline Comparison")
print(f"{'='*80}")
print(f"  Total paths: {total_n:,}")
print(f"  PYs tested: {pys} ({n_pys} years, {n_pys_with_prior} with prior-year data)")
print(f"  Note: aq3 crosses calendar year (Dec PY, Jan-Feb PY+1)")
...
aq3_ex19 = aq3.filter(pl.col("planning_year") >= 2020)
total_ex19 = aq3_ex19.height

results_all = []
for col, label in baselines:
    if col in aq3_ex19.columns:
        results_all.append(eval_baseline(aq3_ex19["mcp_mean"], aq3_ex19[col], label, total_ex19))
...
# C1: Nodal f0 vs H
nh = aq3.filter(pl.col("nodal_f0").is_not_null() & pl.col("mtm_1st_mean").is_not_null())
if nh.height > 0:
    results_c1 = [
...
# C3: q3 vs f0 path vs nodal f0 (where all 3 are available)
q3h = aq3.filter(
    pl.col("q3_path").is_not_null()
    & pl.col("f0_path_corr").is_not_null()
    & pl.col("nodal_f0").is_not_null()
)
if q3h.height > 100:
    results_c3 = [
        eval_baseline(q3h["mcp_mean"], q3h["q3_path"], "q3 path", q3h.height),
        eval_baseline(q3h["mcp_mean"], q3h["f0_path_corr"], "f0 path corr", q3h.height),
        eval_baseline(q3h["mcp_mean"], q3h["nodal_f0"], "Nodal f0", q3h.height),
        eval_baseline(q3h["mcp_mean"], q3h["mtm_1st_mean"], "H", q3h.height),
    ]
    print_main_table(results_c3, f"  C3: q3 vs f0 vs Nodal f0 vs H ({q3h.height:,} all-covered)")
...
for ct in ["onpeak", "offpeak"]:
    sub = aq3.filter(pl.col("class_type") == ct)
    results_ct = []
    for col, label in baselines:
        if col in sub.columns:
...
for lo, hi, bin_label in bins:
    sub = aq3.filter((pl.col("mcp_mean").abs() >= lo) & (pl.col("mcp_mean").abs() < hi))
    if sub.height == 0:
        continue
    results_bin = []
...
actual_arr = aq3_ex19["mcp_mean"].to_numpy().astype(float)
nodal_arr = aq3_ex19["nodal_f0"].to_numpy().astype(float)
f0p_arr = aq3_ex19["f0_path_corr"].to_numpy().astype(float)
h_arr = aq3_ex19["mtm_1st_mean"].to_numpy().astype(float)
...
f0_covered = aq3_ex19.filter(pl.col("f0_path_corr").is_not_null() & pl.col("nodal_f0").is_not_null())
...
# SAVE
# ======================================================================
out_path = f"{WORK_DIR}/aq3_all_baselines.parquet"
aq3.write_parquet(out_path)
print(f"\nSaved: {out_path} ({aq3.shape})")
print(f"Columns: {aq3.columns}")
