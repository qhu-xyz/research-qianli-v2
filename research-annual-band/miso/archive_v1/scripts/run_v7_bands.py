...
"""Bands v7: Bidirectional Calibration + Temporal Everywhere.

v5 achieved 18-44% width reduction via asymmetric bands but failed BG1/BG3/BG4.
v6 tried per-bin inflation correction but widths got WIDER (+8-37% vs v5) and
the one-way correction was nearly inert (0-2 cells adjusted).

v7 introduces OOS-based bidirectional per-bin calibration:
  - Inner temporal holdout: calibrate on all-but-latest training PY,
    measure real OOS coverage on latest training PY
  - Over-covering cells: shrink (lo, hi) inward -> narrower bands
  - Under-covering cells: expand (lo, hi) outward -> wider bands
  - Cold-start folds (< 3 training PYs): multiplicative inflation instead

Key changes from v6:
  A) Temporal CV for ALL rounds (R1 included) — no more LOO primary
  B) min_train_pys=1 — no fold filtering, all data included
  C) Asymmetric only — symmetric dropped (proven inferior in v5)
  D) Bidirectional correction — shrink over-covering + expand under-covering
  E) Explore 6/8/10 bins with and without correction

Experiments per round (identical across R1/R2/R3):
    asym_6b         — asymmetric, 6 bins, no correction
    asym_6b_bidir   — asymmetric, 6 bins, bidirectional correction
    asym_8b         — asymmetric, 8 bins, no correction
    asym_8b_bidir   — asymmetric, 8 bins, bidirectional correction
    asym_10b        — asymmetric, 10 bins, no correction
    asym_10b_bidir  — asymmetric, 10 bins, bidirectional correction
...
Usage:
    ...
    python /home/xyz/workspace/research-qianli-v2/research-annual/scripts/run_v7_bands.py
"""
...
def run_experiment(
    ...
    for test_py in pys:
        ...
        # Calibrate + apply (asymmetric only in v7)
...
def run_round_v7(
    ...
    """Run v7 experiments for one round.
    ...
    print(f"\n{'#'*80}")
    print(f"  ROUND {round_num} — v7 BAND EXPERIMENTS")
    ...
    print(f"\n{'='*70}")
    print(f"  R{round_num} v7 SUMMARY")
    print(f"{'='*70}")
    ...
    for ref_label, ref_path in [("v3 (promoted)", "bands/v3"), ("v5", "bands/v5"), ("v6", "bands/v6")]:
        ref_metrics_path = ROOT / "versions" / ref_path / f"r{round_num}" / "metrics.json"
        if ref_metrics_path.exists():
            with open(ref_metrics_path) as f:
                ref_metrics = json.load(f)
            print(f"\n  vs {ref_label}:")
            print(f"  {'Quarter':<10} {'Ref P95cov':>10} {'v7 P95cov':>10} {'Cov delta':>10} {'Ref hw':>10} {'v7 hw':>10} {'Width chg':>10}")
            print(f"  {'-'*10} {'-'*10} {'-'*10} {'-'*10} {'-'*10} {'-'*10} {'-'*10}")
            for q in QUARTERS:
                ref_cov = ref_metrics.get("coverage", {}).get(q, {}).get("overall", {}).get("p95", {}).get("actual", 0)
                ref_w = ref_metrics.get("widths", {}).get(q, {}).get("overall", {}).get("p95", {}).get("mean_width", 0)
                v7_cov = metrics["coverage"][q]["overall"]["p95"]["actual"]
                v7_w = metrics["widths"][q]["overall"]["p95"]["mean_width"]
                cov_delta = v7_cov - ref_cov
                pct = round((v7_w - ref_w) / ref_w * 100, 1) if ref_w else 0
                print(f"  {q:<10} {ref_cov:>9.2f}% {v7_cov:>9.2f}% {cov_delta:>+9.2f}pp {ref_w:>10.0f} {v7_w:>10.1f} {pct:>+9.1f}%")
...
def main():
    print(f"Bands v7: Bidirectional Calibration + Temporal Everywhere")
    print(f"Memory at start: {mem_mb():.0f} MB")

    # v7 experiment definitions: asymmetric only, 6/8/10 bins, with/without bidir correction
    experiments = [
        {"name": "asym_6b",        "n_bins": 6,  "correction": False},
        {"name": "asym_6b_bidir",  "n_bins": 6,  "correction": True},
        {"name": "asym_8b",        "n_bins": 8,  "correction": False},
        {"name": "asym_8b_bidir",  "n_bins": 8,  "correction": True},
        {"name": "asym_10b",       "n_bins": 10, "correction": False},
        {"name": "asym_10b_bidir", "n_bins": 10, "correction": True},
    ]
