...
"""Bands v9: No Correction, Production-Relevant Evaluation.

v8 used bidirectional OOS correction + cold-start inflation to fix coverage on
early CV folds (1-2 training PYs). But in 2026 production, we train on 6-7 PYs —
cold-start never happens. The correction adds complexity and width (~3-8% wider)
to solve a CV evaluation problem, not a deployment problem.

v8's asym_5b experiment (no correction) already showed that folds with 4+ training
PYs achieve 93-97% P95 coverage. The "failures" come entirely from averaging in
the 1-2 PY folds (84%, 82%, 77%).

v9 = v5-style bands (no correction) with 5 bins and min_train_pys=3 so gate
evaluation excludes cold-start folds.

Method:
  - Asymmetric signed quantile bands with per-(bin, class, sign) calibration
  - No correction (no OOS bidirectional, no cold-start inflation)
  - Temporal CV for ALL rounds
  - min_train_pys=3 (exclude cold-start folds from evaluation)
  - 5 quantile bins

Single experiment per round: asym_5b (asymmetric, 5 bins, no correction)
...
Usage:
    ...
    python /home/xyz/workspace/research-qianli-v2/research-annual/scripts/run_v9_bands.py
"""
...
def run_experiment(
    ...
    """Temporal/LOO experiment runner for asymmetric bands (no correction).
    ...
    Returns same structure as v8's run_experiment (minus correction fields).
    """
    if "sign_seg" not in df.columns:
        df = add_sign_seg(df, baseline_col)
    ...
    for test_py in pys:
        ...
        # Calibrate on full train, no correction
...
def run_round_v9(
    ...
    """Run v9 experiments for one round.

    Each experiment dict: {"name": str, "n_bins": int}
    All rounds use temporal CV, min_train_pys=3.
    Output path: versions/bands/{version_id}/r{round_num}/
    """
    primary_cv = "temporal"

    print(f"\n{'#'*80}")
    print(f"  ROUND {round_num} — v9 BAND EXPERIMENTS")
    ...
    config_data = {
        "schema_version": 1,
        "version": version_id,
        "description": f"R{round_num} asymmetric bands ({winner_exp['n_bins']} quantile bins, class+sign stratified, no correction)",
    ...
    print(f"\n{'='*70}")
    print(f"  R{round_num} v9 SUMMARY")
    print(f"{'='*70}")
    print(f"\n  Winner: {winner_name} (asymmetric, {winner_exp['n_bins']} bins, no correction)")
    ...
    # Compare to prior versions
    for ref_label, ref_path in [("v3 (promoted)", "bands/v3"), ("v5", "bands/v5"), ("v6", "bands/v6"), ("v7", "bands/v7"), ("v8", "bands/v8")]:
        ref_metrics_path = ROOT / "versions" / ref_path / f"r{round_num}" / "metrics.json"
        if ref_metrics_path.exists():
            with open(ref_metrics_path) as f:
                ref_metrics = json.load(f)
            print(f"\n  vs {ref_label}:")
            print(f"  {'Quarter':<10} {'Ref P95cov':>10} {'v9 P95cov':>10} {'Cov delta':>10} {'Ref hw':>10} {'v9 hw':>10} {'Width chg':>10}")
            print(f"  {'-'*10} {'-'*10} {'-'*10} {'-'*10} {'-'*10} {'-'*10} {'-'*10}")
            for q in QUARTERS:
                ref_cov = ref_metrics.get("coverage", {}).get(q, {}).get("overall", {}).get("p95", {}).get("actual", 0)
                ref_w = ref_metrics.get("widths", {}).get(q, {}).get("overall", {}).get("p95", {}).get("mean_width", 0)
                v9_cov = metrics["coverage"][q]["overall"]["p95"]["actual"]
                v9_w = metrics["widths"][q]["overall"]["p95"]["mean_width"]
                cov_delta = v9_cov - ref_cov
                pct = round((v9_w - ref_w) / ref_w * 100, 1) if ref_w else 0
                print(f"  {q:<10} {ref_cov:>9.2f}% {v9_cov:>9.2f}% {cov_delta:>+9.2f}pp {ref_w:>10.0f} {v9_w:>10.1f} {pct:>+9.1f}%")
...
def main():
    print(f"Bands v9: No Correction, Production-Relevant Evaluation")
    print(f"Memory at start: {mem_mb():.0f} MB")

    # v9: single experiment — asymmetric, 5 bins, no correction
    experiments = [
        {"name": "asym_5b", "n_bins": 5},
    ]
