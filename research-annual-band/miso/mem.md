...
1. does Miso have 3 rounds, no distinction of on/offpeak during annual, with each round capable of bidding 4 quarters (aq1 - aq4)?
2. our current strategy:
a. baseline: for round2 and 3, use previous round's mcp (which we also call this round's mtm_1st_mean)
b. banding. for py 2023 for example, for each round, we only consult previous annual's same round data. (round1 only refers to round1)
We use baseline as our base for the band. if we use symmetric banding, then we divide previous years' same round baseline into 4 or 5 bins. --> each bin we use percentile to see to get n percent coverage, how wide the band should be so that for this targer year, we use (baseline - width, baseline + width) to get our band. 
c. to check true coverage: we pull target year's true mcp for this path and see if mcp falls into this band. gather all this year's this round's data and see the overall percentage.

check skills and /pbase if you think my understanding not accurate'

## porting into production
- DO NOT use defualt -> check claude.md for usage. do not fallback silently, do not concfused onpeak and offpeak.
- 