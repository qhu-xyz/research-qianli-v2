"""Comprehensive tests for run_v5_bands.py.

Tests asymmetric calibration, band application, experiment runner,
winner selection, edge cases, and pipeline gate compatibility.

Usage:
    cd /home/xyz/workspace/pmodel && source .venv/bin/activate
    python /home/xyz/workspace/research-qianli-v2/research-annual/scripts/test_v5_bands.py
"""
...
def test_pipeline_gate_structure():
    ...
    # Build metrics dict as run_round_v5 would
    metrics = {
        "coverage": {
...
def test_bg2a_bg2b_gates():
    ...
    # With 7 levels x 2 signs x 4 quarters = 56 combos, degrading prevail p50
    # = 4 combos degraded. 52/56 = 92.9% improved -> PASS (above 90% threshold)
    per_sign_mostly_ok = {}
    for sign in ["prevail", "counter"]:
    ...
    for sign in ["prevail", "counter"]:
        ...
        for lvl in ["p10", "p30", "p50", "p70", "p80", "p90", "p95"]:
            if sign == "prevail" and lvl == "p50":
                # 5% mean degrade, 5% max degrade — 4 combos (one per quarter)
    ...
    if bg2b2:
        # 4/56 degraded = 52/56 = 92.9% improved -> PASS (above 90% threshold)
...
def main():
    print("=" * 70)
    print("  run_v5_bands.py — Comprehensive Test Suite")
    print("=" * 70)
