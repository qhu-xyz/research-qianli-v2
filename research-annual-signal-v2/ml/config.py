...
# ─── Round-sensitive cutoff calendar ─────────────────────────────────────
# Source: pbase MisoApTools._get_cutoff_date_and_settle_duration_for_annual_or_longterm
# These are the round CLOSE dates. History must use data strictly BEFORE this date.

MISO_ANNUAL_ROUND_CLOSE: dict[int, date] = {
    1: date(1, 4, 8),    # April 8  — placeholder year, replaced per PY
...
def get_history_cutoff_month(planning_year: str, market_round: int = 1, rto: str = "miso") -> str:
    ...
    # Last full month = month before the cutoff date's month
    # (unless cutoff is last day of month, but April 7 / April 21 / May 4 are never month-end)
    if cutoff.day < 28:
        # cutoff is mid-month → last FULL month is previous month
        if cutoff.month == 1:
            return f"{cutoff.year - 1}-12"
        return f"{cutoff.year}-{cutoff.month - 1:02d}"
    # cutoff is late in month → this month is complete enough, but be conservative
    if cutoff.month == 1:
        return f"{cutoff.year - 1}-12"
    return f"{cutoff.year}-{cutoff.month - 1:02d}"
