...
| **Spice6 wkndonpeak** | N/A (MISO has no wkndonpeak) | **FILLED** (2026-03-10) — 614 ml_pred files, identical schema to onpeak/dailyoffpeak |
| **Signal names (pmodel)** | `Signal.MISO.DA_V7.16` (f0), `Signal.MISO.DZ_F0P_V5` (f1+) | `Signal.PJM.DA_V7.16` (f0), `Signal.PJM.DZ_F0P_V5` (f1+) |

### wkndonpeak spice6 — FILLED (2026-03-10)

PJM's spice6 `ml_pred` now has all three class types:
...
wkndonpeak was generated using `research-spice-shadow-price-pred/scripts/generate_wkndonpeak_mlpred.py`. Schema and row counts are identical across all three class types.
...
**Columns** (20, PJM-specific — differs from MISO):
```
constraint_id, flow_direction, shadow_price_da, da_rank_value,
density_ori_rank_value, density_mix_rank_value,
ori_mean, mix_mean,
mean_branch_max, mean_branch_max_fillna,
branch_name, bus_key, bus_key_group,
shadow_sign, shadow_price, equipment,
rank_ori, density_mix_rank, rank, tier
```

**Notable PJM-only columns** (not in MISO):
- `branch_name` — PJM branch identifier
- `bus_key`, `bus_key_group` — bus-level keys
- `mean_branch_max`, `mean_branch_max_fillna` — branch thermal limits
- `shadow_sign`, `shadow_price` — raw shadow price info
- `equipment` — facility equipment type

**Notable MISO columns absent from PJM**:
- `density_ori_value`, `density_mix_value` — PJM has rank versions only
- `ori_median`, `ori_max`, `ori_min`, `ori_value_count` — PJM has `ori_mean` only
- `mix_median`, `mix_max`, `mix_min`, `mix_value_count` — PJM has `mix_mean` only
...
aptools = PjmApTools()
df = aptools.tools.get_da_shadow_by_peaktype(st=start_date, et_ex=end_date_exclusive, peak_type=peak_type)
```

**Peak type values**: `onpeak`, `dailyoffpeak`, `wkndonpeak` (from `pbase.data.const.data.ClassType`).
...
### 4.3 constraint_id ↔ DA Shadow Price Join — RESOLVED (2026-03-10)

**Use the branch-level join, NOT the naive monitored_facility join.**

The naive approach (split `constraint_id` on `":"` → `monitored_facility` → join DA) captures only **~46% of DA binding value**. The branch-level join via `constraint_info` captures **96-99%**.

**Verified coverage (branch-level join)**:

| Month | % DA rows matched | % DA value captured |
|-------|:-:|:-:|
| 2022-06 | 93.6% | 96.4% |
| 2023-06 | 98.8% | 99.1% |
| 2024-06 | 97.2% | 99.3% |
...
**How the join works** (reference: `PjmDataLoader.map_constraints_to_branches()` at `research-spice-shadow-price-pred/src/shadow_price_prediction/data_loader.py:805`):

```python
# 1. From constraint_info: constraint_id → monitored_facility (split on ":") → match_str (uppercase)
...
# 3. Interface fallback: for unmatched DA rows, prefix-match against interface match_strs
#    e.g., "BED-BLA CONTINGENCY 24" prefix-matches interface "BED-BLA"
...
# 4. Aggregate realized_sp by branch_name, then join back to all V6.2B rows sharing that branch
data_by_branch = data.groupby("branch_name")[data_ori_cols].sum()
```

**Key details**:
- Case normalization (`.str.upper()`) is required — DA facility names and constraint_info have inconsistent casing
- Interface prefix matching is critical — without it, ~$7-11K/month of interface binding value is lost
- Multiple V6.2B constraint_ids can map to the same branch_name — they all get the same realized_sp target (intentional)
- The ~3% unmatched residual is interface contingencies (e.g., `BED-BLA contingency 24`, `AEP-DOM contingency Bed-Bla`)

**pbase comparison**: `PjmTools.get_da_sp_by_equipment()` at `pbase/analysis/tools/pjm.py:811` does a simpler version (no case normalization, no interface prefix matching) — it's designed for exposure analysis, not ML target construction. Use the research repo's `map_constraints_to_branches()` instead.

See `data-gap-audit.md` §7 for full details.
...
- `density/{class_type}/` — density parquets (NOT class-type partitioned — usable by all 3 class types)
- `score.parquet` — columns: `score`, `constraint_id`, `flow_direction`
- `ml_pred/final_results.parquet` — see below
- `constraint_info/{class_type}/` — constraint metadata (verified only for onpeak)
- `sf/` — raw SF-derived features (enhancement only)

**market_month** = delivery month (= auction_month + N for period type fN).

**ml_pred columns — SAFE to use as features**:
- `predicted_shadow_price`, `binding_probability`, `binding_probability_scaled`
- `prob_exceed_80`, `prob_exceed_85`, `prob_exceed_90`, `prob_exceed_95`, `prob_exceed_100`, `prob_exceed_105`, `prob_exceed_110`
- `constraint_limit`

**ml_pred columns — DO NOT USE (leakage)**:
- `actual_shadow_price` — this IS realized DA, using it is direct leakage
- `actual_binding` — derived from realized data
- `error`, `abs_error` — derived from realized data

**ml_pred class type coverage**: onpeak, dailyoffpeak, and wkndonpeak (all three, 614 files each).

**ml_pred temporal coverage**: 2018-06 through 2026-01 only (not full V6.2B range).
- f0: 92 months (2018-06 to 2026-01)
- f1: 85 months (2018-06 to 2026-01)
- Full availability tables in the companion doc: `research-spice-shadow-price-pred/human-input/pjm-7.0-requirements.md`

### 4.5 Scale Factor (SF) Data

```
# Primary (auto-generated, matches constraint signal schedule):
/opt/data/xyz-dataset/signal_data/pjm/sf/TEST.TEST.Signal.PJM.SPICE_F0P_V6.2B.R1/

# Manual override (sparse):
/opt/data/xyz-dataset/signal_data/pjm/sf/MANUAL.TEST.TEST.Signal.PJM.SPICE_F0P_V6.2B.R1/
```

V7.0 does not modify SF logic — SFs are passed through unchanged. The output signal must maintain the same SF shape and location conventions.
...
### Baseline Signal Availability (from companion requirements doc)
...
| f3–f11 | 80–9 | 2017-06 | varies |

All three class types have the same availability per ptype.

### f0
With 8-month train + 1 lag, earliest eval month ≈ 2018-03.
Realistic dev window: 2018-03 to ~2023-12. Holdout: 2024-01 to 2025-12.

### f1
f1 has **97 months** of data (2017-06 to 2026-03) — NOT sparse like MISO f1.
PJM f1 is available almost every month. Enumerate with `list_available_months()` to confirm exact gaps.
...
## 18. Output Format (CRITICAL)

For ML slices (f0, f1), V7.0 **only replaces 3 columns**:
- `rank_ori` — replaced with ML-derived blended score
- `rank` — recomputed from new rank_ori
- `tier` — recomputed via row-percentile tiering

**All other V6.2B columns must remain unchanged.** The output parquet must have the exact same schema, row universe, and column order as V6.2B. Downstream consumers (pmodel, exposure scaling, etc.) expect the PJM V6.2B shape.

For passthrough slices (f2–f11), the V6.2B parquet is copied unchanged.

---

## 19. Recommended Phased Launch

Not all 6 ML slices have equal data support. Recommended order:

### Wave 1 (best data coverage):
- `f0/onpeak` — full feature stack available
- `f1/onpeak` — full feature stack available

### Wave 2 (good coverage, minor gaps):
- `f0/dailyoffpeak` — ml_pred available, constraint_info may need sharing from onpeak
- `f1/dailyoffpeak` — same

### Wave 3 (highest risk):
- `f0/wkndonpeak` — NO ml_pred data, NO constraint_info
- `f1/wkndonpeak` — same

For Wave 3, decide upfront whether to:
- Use a reduced feature set (drop spice6 ml_pred features, keep bf_* and formula features)
- Keep V6.2B passthrough until ml_pred coverage is available
- Proxy from onpeak ml_pred data (requires validation)

---

## 20. Checklist for the Implementing Agent

- [ ] **Step 0**: Define and validate the true target — read PjmDataLoader.map_constraints_to_branches(), validate join strategy, document with evidence (see §4.3)
- [ ] **Step 1**: Enumerate PJM V6.2B data — list all (month, ptype, ctype) combos, confirm column schema matches §4.1
- [ ] **Step 2**: Validate constraint_id → DA shadow price join — confirm sensible realized_sp distributions
...
| PJM SF data | `/opt/data/xyz-dataset/signal_data/pjm/sf/TEST.TEST.Signal.PJM.SPICE_F0P_V6.2B.R1/` |
| PJM data availability (companion) | `research-spice-shadow-price-pred/human-input/pjm-7.0-requirements.md` |
| PJM shadow price prediction code | `research-spice-shadow-price-pred/src/shadow_price_prediction/data_loader.py` |
| PJM constraint period logic | `pbase/src/pbase/data/const/period/pjm.py` |
| User requirements | `research-pjm-signal-f0p-0/human-input/memory.md` |
