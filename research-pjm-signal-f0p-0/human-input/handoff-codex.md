...
4. `/home/xyz/workspace/research-qianli-v2/research-spice-shadow-price-pred`
...
- `/home/xyz/workspace/research-qianli-v2/research-spice-shadow-price-pred/src/shadow_price_prediction/iso_configs.py`
...
### 8. SPICE6 `ml_pred` is fully usable for PJM (UPDATED 2026-03-10)
...
- `onpeak`: 614 files
- `dailyoffpeak`: 614 files
- `wkndonpeak`: 614 files (generated 2026-03-10 via `research-spice-shadow-price-pred/scripts/generate_wkndonpeak_mlpred.py`)

All three class types have identical schema and row counts. All SPICE6 ML features (`prob_exceed_110`, `constraint_limit`, `binding_probability`, etc.) are now available for all slices.
...
- `wkndonpeak`: 614 files (generated 2026-03-10)

`ml_pred` is a valid candidate feature group for all three class types.

You still need a null-fill or fallback policy for missing rows (e.g., ~2% gap in dailyoffpeak).
...
- `/home/xyz/workspace/research-qianli-v2/research-spice-shadow-price-pred/src/shadow_price_prediction/data_loader.py`
...
- PJM now has `ml_pred` for `wkndonpeak` (generated 2026-03-10), but verify coverage if extending beyond 2026-01.
...
All three class types now have Group D coverage (wkndonpeak ml_pred generated 2026-03-10).
...
6. `wkndonpeak` ml_pred coverage is confirmed (614 files, all features available).
...
- PJM `ml_pred` exists for all three class types: `onpeak`, `dailyoffpeak`, and `wkndonpeak` (614 files each).
- PJM `constraint_info` appears stored only under `class_type=onpeak`.
- PJM DA mapping REQUIRES branch/interface normalization — naive join captures only 46% of value, branch-level join captures 96-99%. See `data-gap-audit.md` §7.
...
- ~~pretending `wkndonpeak` has the same feature coverage as `onpeak`~~ (RESOLVED: wkndonpeak ml_pred now generated)
