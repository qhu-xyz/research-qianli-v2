# Runbook — Shadow Price Classifier Pipeline

Shadow price classification pipeline using XGBoost. Four autonomous agents (orchestrator, worker, Claude reviewer, Codex reviewer) coordinate via file-based handoffs. Runs 3 iterations per batch, fully autonomous after launch. Uses CLI subscriptions only (Claude Max/Pro + ChatGPT Pro) — no API keys required.
...
# With human guidance
echo "Focus on feature engineering for price spikes" > memory/human_input.md
...
```
Pipeline launched in tmux session: pipeline-20260227-142305
Attach:  tmux attach -t pipeline-20260227-142305
Log:     .logs/sessions/pipeline-20260227-142305.log
```
...
| **Worker** | Implements ML changes | `memory/direction_iter{N}.md`, `memory/hot/champion.md`, `memory/hot/learning.md`, `memory/hot/runbook.md` | `ml/`, `registry/{VERSION_ID}/`, handoff JSON | Runs in worktree, must pass tests, must commit before handoff |
...
| Worker | 10 min | 15 min |
| Claude Reviewer | 7 min | 10 min |
| Codex Reviewer | 7 min (non-fatal) | 10 min |
| Orchestrator (synth) | 7 min | 10 min |

### Observed Smoke Timings

Orchestrator ~2.5 min, Worker ~6.5 min, Reviewers ~3 min, Synthesis ~6 min.
...
|---|---|---|
| `PROJECT_DIR` | `.../research-stage1-shadow` | Absolute path to project root |
...
| `TIMEOUT_WORKER` | `900` (15 min) | Hard kill timeout for worker |
...
| `TIMEOUT_SYNTHESIZER` | `600` (10 min) | Hard kill timeout for synthesizer |
