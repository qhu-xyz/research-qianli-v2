        ...
        try:
            ...
            if len(df) > 0:
                ...
                param_cols = [col for col in df.columns if col not in 
                             ['F1', 'Precision', 'Recall', 'MAE', 'RMSE', 'R2', 
...
def sample_params(param_grid: Dict[str, list], 
                  seen_params: Optional[Set[tuple]] = None, 
                  max_retries: int = 1000) -> Dict[str, Any]:
    """
...
def update_config_with_params(config, params):
    ...
    Returns:
        Updated config
    """
    from shadow_price_prediction.config import ModelSpec, ModelConfig
    ...
    config.threshold.beta = params['threshold_beta']
...
def run_single_experiment(iteration, params, test_periods, save_dir):
    ...
    try:
        ...
        config.test_periods = test_periods  # Set test periods
